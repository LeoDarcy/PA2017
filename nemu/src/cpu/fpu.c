...
inline uint32_t internal_normalize(uint32_t sign, int32_t exp, uint64_t sig_grs) {
	...
	if(!overflow) {
		...
		if(for_grs>4)
			...
			for_grs=(sig_grs%2);
		//uint64_t example=sig_grs;
		//example=sig_grs;
		sig_grs+=for_grs;
		if((sig_grs>>23)>1)//need to shift right 
		{
			sig_grs=sig_grs>>1;
			exp++;
			if(exp >= 0xff) {
	...
	sig_grs=sig_grs&0x7FFFFF;//canot be put before during the overflow!!
	FLOAT f;
	f.sign = sign;
